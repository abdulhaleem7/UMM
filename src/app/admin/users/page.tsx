...
export default function AdminUsersPage() {
  ...
  const fetchAdminUsers = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      console.log('[/admin/users] Fetching admin users with token:', token ? 'present' : 'missing');
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('[/admin/users] Admin users response data:', data);
        console.log('[/admin/users] Admin users count:', data.adminUsers?.length || 0);
        setAdminUsers(data.adminUsers || []);
      } else {
        const errorData = await response.text();
        console.error('[/admin/users] Failed to fetch admin users - Response not OK:', response.status, errorData);
      }
    } catch (error) {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      console.log('[/admin/users] Create user response status:', response.status);

      if (response.ok) {
        const responseData = await response.json();
        console.log('[/admin/users] User created successfully:', responseData);
        alert('Admin user created successfully!');
        setShowCreateForm(false);
        setFormData({ username: '', email: '', password: '', role: 'admin' });
        console.log('[/admin/users] Refreshing admin users list...');
        fetchAdminUsers();
      } else {
      ...
      console.error('[/admin/users] Create user exception:', error);
